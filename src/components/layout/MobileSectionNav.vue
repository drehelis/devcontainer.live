...
<template>
  <div class="lg:hidden flex overflow-x-auto bg-ide-activity border-b border-ide-border no-scrollbar sticky top-0 z-30">
    <button
      v-for="s in sections"
      :key="s.id"
    ...
    >
      <span class="text-[10px] font-black uppercase tracking-[0.15em] border-b-2 pb-0.5"
            :class="activeSection === s.id ? 'border-ide-accent' : 'border-transparent'">
        {{ s.name }}
      </span>
    </button>
  </div>
